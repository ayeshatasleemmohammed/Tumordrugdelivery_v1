...
def simulation(case_type):
    ...
    accumulation = np.zeros(no_of_particles,dtype= bool)
    lymphatic_clearence = np.ones(no_of_particles,dtype = bool)
    lymphatic_clearence_probability = 0.0005
    extravastation_probability = 0.0024
    ...
    for t in range(steps):
        ...
        if(case_type == "case2" or case_type=="case3" ):
            ...
            efficiency = np.sum(is_drug_released)/no_of_particles
        elif(case_type == "case4" or case_type == "case5"):
                new_captured = (dis_fro_tumor<=tumor_radius)&(~is_bound)&(alive)
                new_bound = (new_captured)&(binding_roll<binding_probability)
                is_bound[new_bound]=True
                wasted = (probability_of_each_particle<= immune_clearence_probability)&(alive)&(~is_bound)
             ...
             bypassed = (organ_bypass_drug<= organ_bypass)&(dis_fro_tumor>tumor_radius)&alive&~is_bound
             alive[bypassed] = False
             tumor_distance = (dis_fro_tumor<=tumor_radius+1.0e-3)
             extravastation_roll = np.random.rand(no_of_particles)
             new_accumulation = (alive)&(tumor_distance)&(extravastation_roll<=extravastation_probability)&(~accumulation)
             accumulation[new_accumulation] = True
             clearence_roll = np.random.rand(no_of_particles)
             clearence = (clearence_roll<=lymphatic_clearence_probability)&(accumulation)&(~is_drug_released)
             lymphatic_clearence[clearence] = False
             is_drug_released[accumulation&lymphatic_clearence] = True
             efficiency = np.sum(is_drug_released)/no_of_particles       
...
plt.plot(captured_history1,label = f"EPR effect(case1),efficiency={captured_history1[-1]:.4f}",color = "green")
plt.plot(captured_history2,label = f"Targeted Drug delivery(case2),efficiency={captured_history2[-1]:.4f}",color = "pink")
plt.plot(captured_history3,label = f"Targeted Drug delivery+pressure force(case3),efficiency={captured_history3[-1]:.4f}",color = "yellow")
plt.plot(captured_history4,label = f"shrinkage of particles(case4),efficiency={captured_history4[-1]:.4f}",color = "orange")
plt.plot(captured_history5,label = f"shrinkage of particles+pressure force(case5),efficiency={captured_history5[-1]:.4f}",color = "blue")
plt.xlabel("x")
plt.ylabel("y")
plt.legend()
...
plt.show()
plt.figure(figsize=(10,6))
plt.scatter(trajectory_case1[0,:30,0],trajectory_case1[0,:30,1],s = 20,color = "green",label="point of injection")
plt.scatter(trajectory_case1[-1,:30,0],trajectory_case1[-1,:30,1],s = 20,color = "black",label="point of end")
for i in range(30):
     plt.plot(trajectory_case1[:,i,0],trajectory_case1[:,i,1],color ="pink",alpha = 0.5)
